...
- **`100x-value`** — tokens measure *cost*; this measures *value*. The dashboard shows **every directory that consumed tokens** (repo or not) plus every agentic project discovered machine-wide via marker files (CLAUDE.md, AGENTS.md, GEMINI.md, .cursorrules, .windsurfrules, .github/copilot-instructions.md) — even with zero Claude token spend. Value is derived tool-agnostically from git history (commits / PRs / files / churn) with a filesystem-mtime fallback, plus cached AI one-line summaries via the local `claude` CLI. Cost is Claude-Code-only (the only tool with local token accounting); directories from other tools show value with `—` cost, never $0. Four inline-SVG charts render in the same URL: a leverage scatter (value vs cost, break-even line), cost-over-time, token-purpose split, and cost-by-directory.
