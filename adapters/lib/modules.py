...
# Markers + manifest let `emit-claude-code` prune ONLY 100x-dev's own artifacts
# (never the user's hand-authored skills/commands) when a module is removed or
# merged upstream. See cmd_emit_claude_code.
GENERATED_MARKER = ".100x-dev-generated"
ALIAS_MARKER = "<!-- 100x-dev generated alias — regenerate, do not edit -->"
MANIFEST_NAME = ".100x-dev-manifest.json"
...
def render_codex_agents(modules: list[dict]) -> str:
    ...
    out = [
        ...
        "- When the user names a 100x-dev slash workflow like `/gate`, treat it as a request to use the matching skill listed below.\n",
...
def cmd_emit_codex(project_dir: str):
    ...
    if skills_dir.exists():
        for child in skills_dir.iterdir():
            if not child.is_dir():
                continue
            marker = child / ".100x-dev-generated"
            if marker.exists():
                shutil.rmtree(child)
    ...
    for module_dir in sorted(MODULES_DIR.glob("*")):
        ...
        if target.exists():
            marker = target / ".100x-dev-generated"
            if marker.exists():
                shutil.rmtree(target)
            else:
                print(f"skipped existing non-100x-dev skill: {target}", file=sys.stderr)
                continue
        shutil.copytree(module_dir, target)
        (target / ".100x-dev-generated").write_text(
    ...
    for hook in manifest.get("hooks", []):
        ...
        hooks.setdefault(hook["event"], []).append({
            ...
            "hooks": [{
                "type": "command",
                "command": command,
                "statusMessage": f"100x-dev: {hook['description']}",
            }],
        })
